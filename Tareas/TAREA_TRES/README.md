...
Nota: Dentro del `src/main.cpp` de esta primera parte se encuentra un pequeño encabezado que explica que solo basta con cambiar el tipo del objeto para ver los otros dos casos; por defecto, está en `complejo`. Es solo cambiarlo a `int` o `float` para ver los otros. Sería algo como `Matriz<tipo deseado>` para ambas matrices e igualmente para `OperacionesBasicas<Mismo tipo>`
